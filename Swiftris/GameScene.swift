...
class GameScene: SKScene {
    ...
    // ensure that GameViewController need only pass those elements to GameScene in order for them to animate properly
    func animateCollapsingLines(linesToRemove: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>, completion:() -> ()) {
        ...
        for (columnIdx, column) in fallenBlocks.enumerate() {
            for (blockIdx, block) in column.enumerate() {
                ...
                let sprite = block.sprite!
                // produce the pleasing effect for eye balls
        ...
        for (_, row) in linesToRemove.enumerate() {
            for (_, block) in row.enumerate() {
                // make their blocks shoot off the screen like explosive debris
                ...
                let sprite = block.sprite!
                // blocks animate above the other blocks and begin the sequence of actions
        ...
        // drop the last block to its new resting place
